...
\paragraph{Skalierbarkeit als eigenständige Evaluationsdimension.} Mazlan et al.~\cite{mazlan2020} systematisieren die Skalierbarkeitsherausforderungen blockchainbasierter Gesundheitssysteme und unterscheiden dabei Transaktionsaufkommen und Datenvolumen als getrennte Ursachen -- eine Trennung, die in dieser Arbeit unmittelbar zur Aufteilung in die Szenarien S2 und S3 führt. Sehimi und Bendaoud~\cite{sehimibendaoud2023} vertiefen dies für konkrete Lösungsansätze in EHR-Systemen, benennen Durchsatz und Latenz als Standardmetriken und kommen zu dem Ergebnis, dass viele vorgeschlagene Systeme ihre Skalierbarkeit überhaupt nicht oder nicht ausreichend evaluieren; sie fordern daher einheitliche, durchsatz- und latenzbasierte Evaluationsstandards~\cite{sehimibendaoud2023}. Dieser Forderung folgt das Evaluationskonzept dieser Arbeit.

\paragraph{Konsens und Interoperabilität als Vergleichsgegenstand.} Hashim et al.~\cite{hashim2021} evaluieren mehrere Konsensalgorithmen im Kontext des EHR-Austauschs über 100 bis 400 Knoten und zeigen unter anderem, dass die Konsenszeit von PBFT linear mit der Knotenanzahl wächst~\cite{hashim2021}. Die Studie belegt damit, dass der Konsensmechanismus ein eigenständiger, knotenzahlabhängiger Evaluationsgegenstand ist, und liefert Referenzwerte für die Einordnung eines aBFT-Verfahrens gegenüber den in permissioned Netzwerken verbreiteten BFT-Varianten. Carlos Ferreira et al.~\cite{ferreira2024} ergänzen eine PRISMA-basierte Übersicht zu Interoperabilität und Sicherheit und betonen, dass die Interoperabilität zwischen verschiedenen Plattformen selbst eine offene Forschungsfrage ist~\cite{ferreira2024} -- eine Perspektive, die der in dieser Arbeit vorgenommene Austausch der Ledger-Schicht unmittelbar aufgreift.
