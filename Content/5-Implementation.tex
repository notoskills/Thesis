...
Die Tabelle~\ref{tab:impl-startparameter} stellt die Startparameter des Agenten in beiden Varianten gegenüber und löst damit die Aussage ein, die Agentenschicht sei identisch konfiguriert. Sie trifft in der Sache zu, jedoch nicht wörtlich: Transport, Verwaltungsschnittstelle, Wallet-Typ, Selbsteinrichtung und Protokollierungsstufe stimmen überein, während sich drei Gruppen von Parametern unterscheiden. Die erste Gruppe ist durch die Ledger-Anbindung erzwungen und damit gerade der Gegenstand des Vergleichs. Die zweite betrifft die Bezeichner der Wallet, die je Variante verschieden sein müssen, damit beide Aufbauten nebeneinander bestehen können. Die dritte ist die einzige Abweichung, die sich nicht aus der Architektur ergibt: Auf der Indy-Variante ist eine Umgebungsvariable gesetzt, die die Protokollierung der Ledger-Bibliothek auf die Stufe \texttt{debug} hebt, und zu der es auf der HCS-Variante kein Gegenstück gibt. Sie liegt nicht auf dem gemessenen Pfad, wird hier aber ausgewiesen, statt sie in der Aussage der Gleichartigkeit aufgehen zu lassen.
...
Die Tabelle~\ref{tab:impl-szenarioparameter} fasst die Parameter der fünf Szenario-Module zusammen, soweit sie für das Nachvollziehen der berichteten Werte erforderlich sind.
