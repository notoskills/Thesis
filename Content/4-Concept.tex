...
Zwei Punkte bleiben dabei offen. Auf der Wurzelstufe ist jede Änderung des Steward-Kreises zugleich eine Änderung des Topics und nur mit dem \texttt{adminKey} möglich (Abschnitt~\ref{subsec:grundlagen-hcs-topics}): Bleibt dieser ungesetzt, ist der Kreis eingefroren; als einzelner Schlüssel gesetzt, kann sein Inhaber die Schwellenregel aushebeln. Auf der zweiten Stufe prüft das Netzwerk nur den Besitz des objekteigenen Schlüssels und nicht den Delegationsstatus seines Inhabers, sodass die auswertende Komponente zwei zusätzliche Angaben benötigt: ein Merkmal zur Zuordnung einer Nachricht zu ihrem Schreiber und eine zeitliche Bezugsgröße für die Prüfung des Delegationszustands.
...
Die anwendungsseitige Prüfstufe setzt eine Komponente voraus, die von der unveränderten Agentenschicht nicht bereitgestellt wird. Sie würde nicht die verglichene Ledger-Technologie messen, sondern eine eigens für diese Arbeit entstandene Erweiterung, deren Aussagekraft anders zu bewerten wäre als die native Rollenprüfung des Indy-Ledgers. Diese Stufe wird daher -- wie die in Abschnitt~\ref{subsec:e5-leseweg} verworfene Alternative des lokal fortgeschriebenen Zustands -- als Optimierungspotenzial dokumentiert und nicht umgesetzt (Abschnitt~\ref{sec:results-grenzen}). Szenario S5 prüft entsprechend die netzwerkseitige Stufe auf beiden Varianten vollständig, nicht aber die anwendungsseitige Delegation auf der HCS-Variante.
...
\paragraph{Optionen und Bewertung.}
AnonCreds trennt das kryptografische Protokoll vom Speicherort und definiert die Registry als austauschbare Schnittstelle~\cite{anoncredsspec}. Zur Wahl stehen daher eine im Rahmen dieser Arbeit entwickelte Registry- und Resolver-Implementierung und die Verwendung des bestehenden Hedera-Plugins für Aries Cloud Agent Python, das \texttt{did:hedera} auflöst und AnonCreds-Objekte über HCS bereitstellt~\cite{acapyhedera}. Beide lassen die DIDComm- und Peer-DID-Mechanismen unberührt und unterscheiden sich in zwei Punkten: Eine eigene Implementierung belegt die Kompatibilität nach FA6 nur für den eigenen Aufbau, während eine bestehende Erweiterung sie für die reguläre Erweiterungsschnittstelle des Agenten belegt; und sie brächte Eigenschaften ein, die sich in der Messung nicht von denen des Ledgers trennen ließen -- dasselbe methodische Argument, das bereits für E4 entscheidend war.

Das Plugin ist mit E3 und E4 konsistent, da es \texttt{did:hedera} und die Hiero AnonCreds Method umsetzt~\cite{acapyhedera,hasheddidmethod,hieroanoncreds}. Für E5 besteht die Konsistenz darin, dass es selbst keinen Zustand vorhält, sondern den Stand eines Objekts bei jedem Lesezugriff aus der Topic-Historie ableitet. Da dies eine Eigenschaft des Plugins und keine Festlegung der Spezifikation ist, wird sie im Rahmen der Implementierung überprüft; von ihr hängt die Interpretation der nach NFA2 und NFA5 gemessenen Lesevorgänge ab.
...
Für die Lastszenarien gilt darüber hinaus: Beide Varianten werden mit gleicher Knotenanzahl und in gleicher Ausführungsumgebung betrieben, damit nicht Eigenschaften unterschiedlich dimensionierter Netzwerke verglichen werden (NFA6). Die konkrete Festlegung erfolgt im Kapitel~\ref{cha:implementierung}.
...
Schließlich Erhoben werden Zeitstempel an der Registry-Schnittstelle sowie Ressourcenmetriken aller Komponenten innerhalb der Messgrenze.
