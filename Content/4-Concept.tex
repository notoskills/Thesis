...
Die Analyse der Referenzarchitektur im Abschnitt~\ref{sec:konzipierung-referenzarchitektur} sowie die im Kapitel~\ref{cha:state_of_the_art} abgeleiteten Vergleichskriterien bestimmen die Anforderungen an die zu konzipierende HCS-basierte Ledger-Schicht. Die Anforderungen werden in funktionale und nicht-funktionale Anforderungen unterteilt. Die funktionalen Anforderungen ergeben sich direkt aus der Aussage der funktionalen Äquivalenz: Der Proof-of-Concept ersetzt die Ledger-Schicht, ohne die Agenten-Schicht zu verändern, und muss daher dieselben fünf Kernfunktionen bereitstellen, die im Abschnitt~\ref{subsec:ledger-funktionen} definiert sind. Die nicht-funktionalen Anforderungen beschreiben die drei Evaluationsdimensionen dieser Arbeit -- Performanz, Skalierbarkeit und Sicherheit -- und bilden somit die Grundlage für das spätere Evaluationskonzept. Es sollte betont werden, dass sich alle Anforderungen auf die zu ersetzende Ledger-Schicht beziehen und nicht auf das Gesamtsystem; Anforderungen an die Komponenten, die unverändert übernommen wurden, wurden von Erler et al. im Rahmen des Threat-Modeling-Prozesses der Referenzarchitektur berücksichtigt~\cite{erler2023}.
...
\begin{description}
  ...
  \item[NFA2 -- Latenz:] Die Ledger-Schicht muss für jede der im Abschnitt~\ref{subsec:ledger-funktionen} identifizierten Kernfunktionen einen beobachtbaren, klar definierten Zeitpunkt der Finalität bereitstellen, der nach Operationstyp differenziert ist, sodass operationsspezifische Latenzen zwischen Indy und HCS vergleichbar gemessen werden können. Da lesende Zugriffe im Betrieb bei jeder Ausstellung und jeder Präsentation anfallen, schreibende dagegen nur einmalig je Issuer bzw. je Widerruf (Tabelle~\ref{tab:beispiel-ledgerzugriffe}), muss dies für Lese- und Schreiboperationen gleichermaßen gelten.
...
\paragraph{Problem.}
Da HCS keinen Zustand speichert, muss definiert werden, was als Nachricht auf den Ledger geschrieben wird. Für die Gesundheitsdaten ist dies bereits entschieden: Sie bleiben off-chain~\cite{erler2023} und sind nach Abschnitt~\ref{subsec:abgrenzung} nicht Gegenstand des Austauschs. Offen ist die Frage für die vier Objektgruppen der Ledger-Schicht -- öffentliche DIDs, Schemas, Credential Definitions und Widerrufsinformationen. Bei Indy stellt sie sich nicht, da der Domain Ledger sie als native Objekttypen hält (Abschnitt~\ref{subsec:grundlagen-speicher-indy}). Auf HCS entsteht sie, weil eine Nachricht auf 1024 Byte begrenzt ist (Abschnitt~\ref{subsec:grundlagen-speicher-hcs}), während Credential Definitions und Revocation Registry Definitions diese Grenze deutlich überschreiten; die Hiero AnonCreds Method -- Hiero ist das unter der Linux Foundation geführte Open-Source-Projekt, in das die Hedera-Software überführt wurde -- dokumentiert ein Beispiel, in dem eine CredDef auf sechs und eine RevRegDef auf zwei Nachrichten aufgeteilt werden~\cite{hieroanoncreds}, wobei die Nachrichtenzahl mit der Attributenzahl des Schemas wächst.

\paragraph{Optionen und Bewertung.}
Der Optionsraum ergibt sich aus drei Speichermustern der Literatur und der HCS-Spezifikationen. Die erste Option ist die vollständige Kodierung jeder Zustandsänderung als Nachricht, aus der Anwendungen den Zustand durch Replay des geordneten Stroms rekonstruieren -- das von Baird et al.\ beschriebene Appnet-Modell, bei dem die Konsensknoten ordnen, zeitstempeln und verwerfen, während Mirror Nodes und abonnierende Mitglieder die Historie behalten~\cite{bairdgross2019}; die Hedera-DID-Methode wendet es an und definiert die Auflösung als Verarbeitung der Nachrichten in Konsensreihenfolge~\cite{hasheddidmethod}. Die zweite Option ist das Hash-Anchoring, bei dem das Objekt außerhalb des Ledgers liegt und nur ein Hashwert konstanter Größe verankert wird; in den untersuchten Umsetzungen ist es das dominierende Muster~\cite{mazlan2020, farin2025, tawfik2025, mamoru2026}. Die dritte Option ist die Ablage über den HCS-1-Standard, der größere Objekte auf mehrere Nachrichten eines dedizierten Topics verteilt~\cite{hcs1standard}.

Erler et al.~\cite{erler2023} nennen für die Ledger-Schicht genau vier Objektgruppen -- öffentliche DIDs, Schemas, Credential Definitions und eine Revocation Registry --, während die Gesundheitsdaten off-chain bleiben. Für das Hash-Anchoring spricht allein das Größenproblem, das jedoch bereits HCS-1 innerhalb des Ledgers löst; die Motivation entfällt damit, die Nachteile bleiben. Hinzu kommt, dass die SSI-basierten Arbeiten für Identitätsmetadaten umgekehrt verfahren und sie vollständig auf dem Ledger ablegen, da dort ausschließlich öffentlich sichtbare Objekte liegen~\cite{torongo2023,manoj2022}. Den Ausschlag geben drei Anforderungen: FA3 und FA4 verlangen den Abruf in einer dem AnonCreds-Datenmodell entsprechenden Form~\cite{anoncredsspec}, was beim Hash-Anchoring die zusätzliche Definition einer externen Auflösung erforderte; FA5 fordert eine dauerhaft abrufbare, manipulationsresistente Historie, während das Hash-Anchoring nur den Hashwert sichert und Verfügbarkeit wie Integrität des referenzierten Objekts außerhalb des Konsenses lässt. Die vollständige Kodierung entspricht dagegen allen drei Anforderungen unmittelbar und liefert nach Satybaldy et al.\ zudem die niedrigste gemessene Auflösungslatenz aller untersuchten DID-Methoden~\cite{satybaldy2026}.
...
\paragraph{Optionen und Bewertung.}
Da HCS als Ledger festgelegt ist, entfallen die ledgerbasierten Methoden anderer Plattformen~\cite{satybaldy2026}. Es verbleiben die ledgerlosen Methoden, für die \texttt{did:key} steht, und die HCS-gestützte Methode \texttt{did:hedera}. Weitere verbreitete Methoden kommen für die Rolle eines öffentlichen Vertrauensankers nicht in Betracht: \texttt{did:peer} ist ausdrücklich nicht öffentlich auflösbar und bereits für die paarweisen Verbindungen der Agentenschicht vorgesehen, und \texttt{did:web} verankert die Identität in einem Webserver und dessen Domain statt in einem konsensgesicherten Ledger. Bei \texttt{did:key} wird der Identifikator vollständig aus dem Schlüsselmaterial abgeleitet und das DID-Dokument lokal erzeugt; eine öffentliche Registrierung, eine Schlüsselrotation und eine Deaktivierung sind nicht vorgesehen~\cite{didkeyspec}. Damit kann die Methode weder die Vertrauensanker-Funktion übernehmen noch den in FA2 geforderten Lebenszyklus abbilden. Bei \texttt{did:hedera} werden Änderungen am DID-Dokument als Nachrichten auf einem HCS-Topic veröffentlicht, dessen Kennung Bestandteil des Identifikators ist, sodass die Auflösung über Mirror Nodes öffentlich möglich ist~\cite{hasheddidmethod}.

\paragraph{Entscheidung.}
Verwendet wird \texttt{did:hedera}. Das entspricht dem Muster der SSI-basierten Arbeiten, die DID-Methode vom eingesetzten Ledger abhängig zu machen~\cite{erler2023, manoj2022, torongo2023}, und hält die Ersetzung auf die Ledger-Schicht beschränkt: Die DID-Dokumente beider Methoden folgen dem W3C-Datenmodell~\cite{w3cdidcore}, sodass die Agentenschicht sie über ihre Resolver-Schnittstelle unverändert verarbeitet, auch wenn sich Bezeichnerformat und Auflösungsweg unterscheiden.
...
\paragraph{Optionen und Bewertung.}
Der Optionsraum ergibt sich aus den beiden Zugriffsarten einer Mirror Node: die Auflösung je Lesezugriff, bei der die auswertende Komponente selbst keinen Zustand hält und bei jeder Anfrage die Topic-Historie abruft, und das Appnet-Muster, bei dem sie den Nachrichtenstrom dauerhaft abonniert, den Zustand lokal fortführt und Lesezugriffe daraus beantwortet~\cite{bairdgross2019}.

Für NFA2 unterscheiden sich beide grundlegend: Bei Auflösung je Lesezugriff wächst der Aufwand mit der Länge der Historie, bei fortgeschriebenem Zustand ist er davon unabhängig und verlagert sich auf die laufende Fortschreibung. Satybaldy et al.\ führen die von ihnen gemessene niedrige Latenz gerade darauf zurück, dass die Historie eines Topics isoliert ausgewertet werden kann~\cite{satybaldy2026} -- für die nach NFA5 zu untersuchende Entwicklung bei wachsendem Bestand ist jedoch genau diese Abhängigkeit der interessierende Effekt. Für FA5 ist maßgeblich, dass die Konsensknoten den Nachrichteninhalt nach der Ordnung verwerfen~\cite{bairdgross2019}: Beide Optionen setzen deshalb eine Mirror Node voraus, die damit keine externe Infrastruktur ist, sondern Teil der gemessenen Systemgrenze und der erste Kandidat für den nach NFA4 zu lokalisierenden Engpass.

Den Ausschlag geben drei Punkte. Erstens bleibt die Agentenschicht nach FA6 unverändert und erzeugt die Lesezugriffe selbst; eine zusätzliche Zustandskomponente stünde nicht auf dem tatsächlichen Lesepfad, sondern neben ihm. Zweitens ist die Abhängigkeit der Lesedauer von der Historienlänge genau der Unterschied zwischen Zustandsspeicher und Ordnungsdienst, den diese Arbeit messen soll; ein fortgeschriebener Zustand beseitigte diesen Effekt, bevor er quantifiziert ist. Drittens wäre eine nur auf der HCS-Seite eingeführte Zustandskomponente kein symmetrischer Versuchsaufbau, da auf der Indy-Variante keine entsprechende Komponente hinzukommt.
...
\paragraph{Problem.}
FA1 erfordert eine Vertrauenshierarchie mit mindestens zwei Berechtigungsstufen, in der nur autorisierte Akteure schreiben dürfen. In Indy definiert der Ledger diese Hierarchie selbst: Nur Akteure mit entsprechender Rolle können gültige NYM-Transaktionen einreichen~\cite{torongo2023}. HCS kennt keine rollenbasierte Schreibberechtigung. Zu entscheiden ist zunächst der Mechanismus, davon abhängig die Topic-Struktur, auf der die Hierarchie geführt wird -- letztere betrifft ausschließlich die Hierarchie selbst, da die Topics der DID-Registry und der AnonCreds-Objekte durch E3 und E4 bereits vorgegeben sind~\cite{hasheddidmethod,hieroanoncreds}.

\paragraph{Optionen und Bewertung.}
Der Optionsraum lässt sich nicht aus der Literatur ableiten, da keine der untersuchten Arbeiten eine Vertrauenshierarchie auf HCS umsetzt; er ergibt sich aus zwei dokumentierten Kontrollpunkten und ihrer Kombination. Der erste ist der netzwerkseitige \texttt{submitKey} eines Topics, der auch als Threshold Key ausgeprägt sein kann (Abschnitt~\ref{subsec:grundlagen-hedera-plattform}). Der zweite ist die anwendungsseitige Prüfung im Appnet-Modell, bei der die auswertende Anwendung Nachrichten ohne gültigen Rollennachweis verwirft~\cite{bairdgross2019}.

Die Literatur zeigt für die Ausgestaltung durchgehend dasselbe Muster: eine netzwerkseitig verankerte Wurzel mit einer delegierten zweiten Stufe, wobei über die Wurzel ein Gremium und nicht ein einzelner Akteur entscheidet~\cite{torongo2023, manoj2022, joshimahajan2025, tawfik2025}. Ein Threshold Key bildet diese Wurzel am genauesten ab, weil er kollektives Entscheiden unmittelbar auf den Signaturmechanismus überträgt und netzwerkseitig durchgesetzt wird; die delegierte Stufe lässt sich damit jedoch nicht ausdrücken, da der \texttt{submitKey} nur zwischen berechtigt und unberechtigt unterscheidet. Eine rein anwendungsseitige Prüfung gäbe umgekehrt die netzwerkseitige Verankerung der Wurzel auf, womit die geforderte Vertrauensanker-Funktion~\cite{erler2023} allein von der korrekten Implementierung der Leseseite abhinge. Die Kombination beider erfüllt FA1 und NFA7 am vollständigsten, da eine unberechtigte Nachricht entweder vom Netzwerk abgewiesen oder von der auswertenden Komponente verworfen wird und in beiden Stufen ein beobachtbares Ergebnis vorliegt.

Zwei Punkte bleiben dabei offen. Auf der Wurzelstufe ist jede Änderung des Steward-Kreises zugleich eine Änderung des Topics und nur mit dem \texttt{adminKey} möglich (Abschnitt~\ref{subsec:grundlagen-hcs-topics}): Bleibt dieser ungesetzt, ist der Kreis eingefroren; als einzelner Schlüssel gesetzt, entscheidet dessen Inhaber allein über die Schwellenregel. Auf der zweiten Stufe prüft das Netzwerk nur den Besitz des objekteigenen Schlüssels und nicht den Delegationsstatus seines Inhabers, sodass die auswertende Komponente zwei zusätzliche Angaben benötigt: ein Merkmal zur Zuordnung einer Nachricht zu ihrem Schreiber und eine zeitliche Bezugsgröße für die Prüfung des Delegationszustands.

Für die Topic-Struktur geben E3 und E4 bereits vor, dass je öffentlicher DID und je AnonCreds-Objekt ein eigenes Topic entsteht. Daraus folgt eine für FA5 wesentliche Einschränkung: HCS garantiert eine Totalordnung ausschließlich innerhalb eines Topics~\cite{bairdgross2019}, sodass eine übergreifende Historie nachträglich aus den Konsens-Zeitstempeln rekonstruiert werden muss.

\paragraph{Entscheidung.}
Die Vertrauenshierarchie wird auf einem einzelnen Topic geführt, dessen \texttt{submitKey} als Threshold Key von den Steward-Instanzen gemeinsam kontrolliert wird; der \texttt{adminKey} erhält dieselbe Schwellenregel, damit der Steward-Kreis änderbar bleibt, ohne dass eine einzelne Instanz die Wurzel übernehmen kann. Endorser-Rechte werden über Delegationsnachrichten auf demselben Topic vergeben und entzogen; da der \texttt{submitKey} den Schreibzugriff auf den Steward-Kreis beschränkt, ist ihre Herkunft bereits durch die Annahme der Nachricht belegt, und die Hierarchie umfasst genau die geforderten zwei Stufen, weil Endorser weder sich selbst entziehen noch weiter delegieren können.

Die Gültigkeit prüft der Registry-Adapter als auswertende Komponente des in E5 festgelegten Lesewegs, indem er die Historie des Vertrauens-Topics bei jeder Prüfung erneut ausliest -- und zwar gegen den Delegationszustand zum Konsens-Zeitstempel der bewerteten Objektnachricht, sodass ein späterer Entzug bereits geschriebene Objekte nicht rückwirkend entwertet und das Verhalten dem von Indy entspricht~\cite{torongo2023}. Ist der Delegationszustand nicht lesbar, gilt die Nachricht als ungültig. Die übrigen Festlegungen -- die Besetzung der Schwellenregel im Versuchsaufbau und das Merkmal, das die Zuordnung einer Nachricht zu ihrem Schreiber trägt -- sind für die gemessenen Eigenschaften ohne Belang und werden in Kapitel~\ref{cha:implementierung} getroffen.

Umgesetzt wurde dieser Entwurf im Prototyp nicht, und zwar keine seiner beiden Stufen. Das Vertrauens-Topic mit einem von den Steward-Instanzen gemeinsam kontrollierten Threshold Key setzt eine Einrichtung der Wurzel voraus, die das verwendete Plugin nicht vorsieht; die anwendungsseitige Delegationsprüfung setzt eine Komponente voraus, die von der unveränderten Agentenschicht nicht bereitgestellt wird. Beide würden nicht die verglichene Ledger-Technologie messen, sondern eine eigens für diese Arbeit entstandene Erweiterung, deren Aussagekraft anders zu bewerten wäre als die native Rollenprüfung des Indy-Ledgers. Der Entwurf wird daher -- wie die in Abschnitt~\ref{subsec:e5-leseweg} verworfene Alternative des lokal fortgeschriebenen Zustands -- dokumentiert, aber nicht umgesetzt (Abschnitt~\ref{sec:results-grenzen}). Im Prototyp existiert auf der HCS-Variante damit ausschließlich der objektgebundene \texttt{submitKey}, den die in E3 und E4 gewählten Methoden je Objekt-Topic setzen. Szenario S5 prüft entsprechend auf der HCS-Variante diesen objektgebundenen Schlüsselschutz und auf der Indy-Variante das Rollenmodell, nicht aber eine Vertrauenshierarchie auf der HCS-Variante.
...
\paragraph{Optionen und Bewertung.}
AnonCreds trennt das kryptografische Protokoll vom Speicherort und definiert die Registry als austauschbare Schnittstelle~\cite{anoncredsspec}. Zur Wahl stehen daher eine im Rahmen dieser Arbeit entwickelte Registry- und Resolver-Implementierung und die Verwendung des bestehenden Hedera-Plugins für Aries Cloud Agent Python (ACA-Py), das \texttt{did:hedera} auflöst und AnonCreds-Objekte über HCS bereitstellt~\cite{acapyhedera}. Beide lassen die DIDComm- und Peer-DID-Mechanismen unberührt und unterscheiden sich in zwei Punkten: Eine eigene Implementierung belegt die Kompatibilität nach FA6 nur für den eigenen Aufbau, während eine bestehende Erweiterung sie für die reguläre Erweiterungsschnittstelle des Agenten belegt; und sie brächte Eigenschaften ein, die sich in der Messung nicht von denen des Ledgers trennen ließen -- dasselbe methodische Argument, das bereits für E4 entscheidend war.

Das Plugin ist mit E3 und E4 konsistent, da es \texttt{did:hedera} und die Hiero AnonCreds Method umsetzt~\cite{acapyhedera,hasheddidmethod,hieroanoncreds}. Für E5 kommt es darauf an, ob das Plugin den Stand eines Objekts bei jedem Lesezugriff aus der Topic-Historie ableitet oder einen eigenen Zustand vorhält. Da dies eine Eigenschaft des Plugins und keine Festlegung der Spezifikation ist, wird sie im Rahmen der Implementierung überprüft (Abschnitt~\ref{subsec:impl-wiederholungen}); von ihr hängt die Interpretation der nach NFA2 und NFA5 gemessenen Lesevorgänge ab.
...
\begin{description}
  ...
  \item[Registry-Adapter:] Bildet die Lese- und Schreiboperationen der Verifiable-Data-Registry-Schnittstelle von AnonCreds auf Topic-Operationen ab und leitet den Zustand eines Objekts bei jedem Lesezugriff aus der über die Mirror Node abgerufenen Historie ab (E5, E7); dabei prüft er nach dem Entwurf die Delegationskette gegen das Vertrauens-Topic (E6); diese Prüfung ist im Prototyp nicht umgesetzt.
  \item[Agentenschicht:] Unverändert übernommen (Abschnitt~\ref{subsec:abgrenzung}); nicht Teil der Ledger-Schicht.
\end{description}

Für das Zusammenspiel dieser Komponenten ist eine Eigenschaft festzuhalten, die die HCS-basierte Variante von der Referenzvariante unterscheidet: Auf dem Schreibweg ist keine zustandsführende Komponente beteiligt. Das Netzwerk ordnet die Nachricht, versieht sie mit einem Konsens-Zeitstempel und stellt sie über die Mirror Node bereit. Eine netzwerkseitige Prüfung der Vertrauenshierarchie findet nach dem Entwurf ausschließlich auf dem Vertrauens-Topic statt, dessen Schreibberechtigung nach E6 gesetzt ist; die Topics der öffentlichen DIDs und der AnonCreds-Objekte sind zwar ebenfalls schlüsselgeschützt, jedoch objektgebunden und unabhängig von dieser Hierarchie. Ob ein schreibender Akteur zum Zeitpunkt des Schreibens delegiert war, würde deshalb erst beim Lesen ausgewertet (E5). Daraus folgen zwei Unterschiede zu Indy. Der Aufwand des Lesewegs wächst mit der Länge der Topic-Historie -- ein Zusammenhang, den die Evaluation nach NFA2 und NFA5 untersucht. Und während Indy einen unberechtigten Schreibvorgang bereits bei der Einreichung abweist, nimmt die HCS-Variante jede Nachricht an, die den objektgebundenen Schlüssel erfüllt; diese Asymmetrie ist Gegenstand des Szenarios S5.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.88\textwidth]{Figures/zielarchitektur}
...
Für die Einzeloperationen nach NFA2 werden Median und robuste Streuung berichtet. Das 95.~Perzentil (p95) -- derjenige Wert, unter dem 95\,\% der gemessenen Antwortzeiten liegen -- wird nur im Lastszenario S2 verwendet, und zwar als Abbruchkriterium; es erfasst einen typischen ungünstigen Fall, ohne dass ein einzelner Ausreißer das Ergebnis dominiert.
...
Verglichen werden zwei Varianten: die Indy-basierte Referenzvariante und die HCS-basierte Untersuchungsvariante. Beide werden mit derselben, unveränderten Agentenschicht betrieben (FA6), sodass gemessene Unterschiede der Ledger-Schicht zugeordnet werden können (Abschnitt~\ref{subsec:abgrenzung}).

Je Variante wird ein einzelner Aries-Agent betrieben, unabhängig davon, wie viele Agenten in einer realen Aries-Instanz gleichzeitig auf die Ledger-Schicht zugreifen und untereinander über DIDComm kommunizieren. Diese Reduktion ist zulässig, weil die Agentenzahl kein Merkmal der VDR-Schnittstelle ist: Jeder Agent löst dieselbe endliche Menge an Ledger-Operationen aus, die im Abschnitt~\ref{subsec:ledger-funktionen} festgelegt wurde, und die Schnittstelle unterscheidet nicht, von wie vielen oder welchen Agenten ein einzelner Aufruf stammt. Was mit der Agentenzahl tatsächlich variiert, ist die Nebenläufigkeit gleichzeitiger Zugriffe auf dieselbe Ledger-Schicht; dieser Effekt wird nicht über eine Vervielfachung der Agenten, sondern über die in S2 gesteigerte Zugriffslast abgebildet (Abschnitt~\ref{subsec:eval-szenarien}).

Als Messgrenze wird die Verifiable-Data-Registry-Schnittstelle festgelegt. Innerhalb der Grenze liegen in der HCS-Variante der Registry-Adapter, das Konsensnetzwerk und die Mirror Node (Abbildung~\ref{fig:zielarchitektur}), in der Indy-Variante der Resolver und die Validatorknoten. Die Agentenschicht liegt in beiden Varianten außerhalb. Diese Festlegung folgt aus der Zielarchitektur und nicht aus der Literatur, da die untersuchten Evaluationen uneinheitliche Systemgrenzen verwenden (Abschnitt~\ref{subsec:eval-messgroessen}).

Für die Lastszenarien gilt darüber hinaus, dass beide Varianten mit gleicher Knotenanzahl und auf demselben Rechner betrieben werden, damit nicht Eigenschaften unterschiedlich dimensionierter Netzwerke verglichen werden (NFA6). Die konkrete Festlegung erfolgt im Kapitel~\ref{cha:implementierung}.

Die Last wird ausschließlich über die Registry-Schnittstelle erzeugt, sodass die DIDComm-Protokolle der Agentenschicht nicht in die Messung eingehen.

Erhoben werden schließlich Zeitstempel an der Registry-Schnittstelle; Ressourcenmetriken der Komponenten innerhalb der Messgrenze waren zusätzlich vorgesehen, wurden jedoch nicht erhoben (Abschnitt~\ref{sec:results-ressourcen}).
...
\footnotesize
  ...
  \begin{tabular}{p{0.20\textwidth} p{0.52\textwidth} p{0.16\textwidth}}
    ...
    \midrule
    S1 -- Basismessung & Sequenzielle Ausführung je einer Schreib- und einer Leseoperation für die Registrierung von DID, für Schema und Credential Definition sowie für das Widerrufsregister, jeweils unter definierter Grundlast. Der Vertrauensanker bleibt ausgenommen, da die Vertrauenshierarchie auf der HCS-Variante im Prototyp nicht umgesetzt ist (Abschnitt~\ref{subsec:e6-vertrauenshierarchie}) und er dort damit keine ausführbare Operation besitzt und ein einseitig erhobener Wert eine Vergleichbarkeit vortäuschen würde; die manipulationsresistente Historie ist eine Eigenschaft und keine diskrete Operation & NFA1, NFA2 \\
    ...
    S3 -- Datenwachstum & Schrittweiser Aufbau eines wachsenden Bestands unbeteiligter Identitätsobjekte mit Lesung eines festen Zielobjekts und einer Kontrolllesung an definierten Zwischenständen; die Erfassung des Speicherbedarfs war zusätzlich vorgesehen & NFA5, NFA2 \\
    S4 -- Knotenausfall & Gezielter Ausfall einzelner Konsensknoten unter einer für dieses Szenario festgelegten Last, mit Beobachtung von Verfügbarkeit, Latenz und Wiederaufnahme & NFA6 \\
    S5 -- Zugriffskontrolle & Nicht autorisierte Schreibversuche -- auf der HCS-Variante gegen den objektgebundenen Schlüsselschutz der Objekt-Topics, auf der Indy-Variante gegen das Rollenmodell --, ergänzt um Nachweise zur delegierten Rolle und zum Widerrufspfad auf der Indy-Variante & NFA7 \\
    \bottomrule
  \end{tabular}
\end{table}
...
Zwei Eigenschaften des Entwurfs sind dabei gesondert zu beachten, weil sie die Aussagekraft der beiden Szenarien begrenzen. Erstens werden durch die vollständige Kodierung der Identitätsobjekte (E2) sämtliche Objektinhalte über die Mirror Node für jeden Leseberechtigten sichtbar~\cite{satybaldy2026}; dies ist keine Messgröße, sondern eine Struktureigenschaft, die bei der Bewertung der Ergebnisse mitzulesen ist. Zweitens ist die in E6 entworfene Vertrauenshierarchie im Prototyp nicht umgesetzt. Das Szenario S5 prüft auf der HCS-Variante deshalb ausschließlich den objektgebundenen Schlüsselschutz der Objekt-Topics und auf der Indy-Variante das Rollenmodell des Ledgers; eine Aussage über die Wirksamkeit einer rekonstruierten Hierarchie auf der HCS-Variante ist nicht möglich.


\subsection{Validität und Gültigkeitsgrenzen}
\label{subsec:eval-validitaet}

Das Evaluationskonzept ist an mehreren Stellen enger gefasst als der Entwurf. Die folgenden Grenzen werden vorab festgehalten, damit die im Kapitel~\ref{cha:results} berichteten Werte im richtigen Rahmen gelesen werden.

Ob der in E4 festgelegte Widerrufsmechanismus messend oder ausschließlich auf Entwurfsebene bewertet wird, hängt von einer Symmetriebedingung ab: Ein allein auf der HCS-Variante erhobener Wert wäre kein Vergleich, sondern eine Einzelmessung. Der AnonCreds-Widerruf ist an die Protokolle zur Ausstellung und Präsentation gebunden und steht deshalb nur dann vergleichbar zur Verfügung, wenn beide Varianten mit derselben Agentenversion und damit demselben Protokollstand betrieben werden. Ob diese Bedingung erfüllt ist, wird im Kapitel~\ref{cha:implementierung} festgestellt. Unabhängig davon bleibt FA4 Bestandteil des Entwurfs, und NFA7 wird zweigeteilt eingelöst: die Zugriffskontrolle experimentell über S5, der Widerruf mindestens über die Prüfung, ob der Akkumulatorzustand nach E4 eindeutig auswertbar veröffentlicht wird.

Weiter gelten die berichteten Werte nur für das jeweils festgelegte Lastprofil -- für S1 und S3 die im Kapitel~\ref{cha:implementierung} definierte Grundlast, für S2 und S4 die dort gesondert festgelegte Last des jeweiligen Szenarios. Auf einem vollständig leerlaufenden Netzwerk weicht die HCS-Variante beim Lesen unmittelbar nach einem Schreibvorgang davon ab (Abschnitt~\ref{subsec:eval-szenarien}); dieser Fall wird gesondert erhoben und geht nicht in den Vergleich ein.

Alle Werte gelten für das nach E1 selbst betriebene Netzwerk. Sie sind nicht auf das öffentliche Hedera-Netzwerk übertragbar, das sich in Knotenmenge, geografischer Verteilung, Fremdlast und Gebührenmodell unterscheidet; insbesondere ist die erreichbare Last dort nicht allein von der Technologie bestimmt.

Der Speicherbedarf je Konsensknoten nach NFA5 ist für beide Varianten vorgesehen, wäre jedoch keine symmetrische Größe. Die Konsensknoten von HCS verwerfen den Nachrichteninhalt nach der Ordnung~\cite{bairdgross2019} -- eine Eigenschaft des Dienstes, nicht der Entscheidung E5 --, sodass dort kein bestandsabhängiges Wachstum zu erwarten ist, während die Validatorknoten von Indy den abgeleiteten Zustand replizieren. Der Unterschied wäre ein Ergebnis des Entwurfs und kein Messartefakt und wäre entsprechend zu berichten. Ob diese Größe tatsächlich erhoben wurde, weist Abschnitt~\ref{sec:results-ressourcen} aus.

Zuletzt gelten die gemessenen Lesewerte nur für den in E5 gewählten Leseweg. Ein lokal fortgeschriebener Zustand nach dem Appnet-Muster würde die Abhängigkeit von der Historienlänge beseitigen; das damit nicht ausgeschöpfte Optimierungspotenzial wird in Abschnitt~\ref{sec:results-grenzen} anhand der gemessenen Werte eingeordnet.
