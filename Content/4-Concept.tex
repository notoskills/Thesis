...
Den Ausschlag geben drei weitere Punkte. Erstens bleibt die Agentenschicht nach FA6 unverändert, und die Lesezugriffe auf die Ledger-Schicht entstehen ausschließlich innerhalb dieser Agentenschicht bei der Ausstellung und Prüfung von Credentials. Eine zusätzliche Zustandskomponente würde damit nicht auf dem tatsächlichen Lesepfad stehen, sondern neben ihm: Sie würde einen zweiten, in der Referenzarchitektur nicht vorgesehenen Zugriffsweg einführen und damit über den im Abschnitt~\ref{subsec:abgrenzung} festgelegten Austauschumfang hinausgehen. Zweitens ist die Abhängigkeit der Lesedauer von der Länge der Topic-Historie genau der Unterschied zwischen einem Zustandsspeicher und einem reinen Ordnungsdienst, den diese Arbeit messen soll. Ein lokal fortgeschriebener Zustand würde diesen Effekt beseitigen, bevor er quantifiziert ist; das Messergebnis wäre dann keine Eigenschaft der Ledger-Technologie mehr, sondern eine Eigenschaft der eigenen Optimierung. Drittens wäre eine allein auf der HCS-Seite eingeführte Zustandskomponente kein symmetrischer Versuchsaufbau. Die Indy-Variante wird unverändert und ohne vorgelagerten Zwischenspeicher betrieben; ein lokal fortgeschriebener Zustand nur auf der HCS-Seite würde deshalb nicht zwei Ledger-Technologien vergleichen, sondern eine optimierte mit einer nicht optimierten Variante. Die Symmetrie würde sich nur herstellen lassen, indem dieselbe Komponente auch der Indy-Variante vorgelagert würde, womit jedoch beide Messreihen Eigenschaften dieser Komponente und nicht mehr der verglichenen Ledger wären. Damit wendet E5 dieselbe Regel an, die bereits in E4 den Ausschlag gegeben hat und die in E7 erneut greift: Wo ein eigener Zusatz gegen den unveränderten oder spezifizierten Mechanismus steht, entscheidet die Zuordenbarkeit der Messergebnisse nach Abschnitt~\ref{subsec:abgrenzung}.

\paragraph{Entscheidung.}
Es wird kein Zustand vorgehalten; jeder Lesezugriff wird durch Abruf und Auswertung der Topic-Historie über die Mirror Node beantwortet. Der lokal fortgeschriebene Zustand nach dem Appnet-Muster wird als Alternative verworfen, bleibt jedoch als Optimierungsmöglichkeit dokumentiert und wird im Kapitel~\ref{sec:discussion} anhand der gemessenen Werte beziffert. Der so festgelegte Leseweg ist damit ausdrücklich eine bewusst nicht optimierte Referenzkonfiguration: Er bildet den Zugriffsweg ab, den die Ledger-Technologie ohne zusätzliche anwendungsseitige Mittel bereitstellt, und nicht die bestmöglich erreichbare Konfiguration; die berichteten Lesewerte sind entsprechend als obere Schranke des Aufwands und nicht als Leistungsgrenze von HCS zu lesen (Abschnitt~\ref{subsec:eval-validitaet}). Für die Evaluation folgt daraus, dass die Länge der Topic-Historie ein bewusst zu setzender Versuchsparameter und kein Nebeneffekt ist: Der zu messende Zusammenhang zwischen Datenbestand und Lesedauer wird nur sichtbar, wenn die Bestände hinreichend groß gewählt werden (Abschnitt~\ref{sec:konzipierung-evaluationskonzept}). Die Mirror Node und die von ihr vorgehaltene Historie bilden zusammen mit dem Konsensnetzwerk und dem Registry-Adapter die Ledger-Schicht im Sinne der Messung.
...
Gemessen an diesem Muster ist die Wurzel keine Eigenschaft eines einzelnen Akteurs, sondern eines Gremiums - bei Torongo und Toorani entscheidet ein Governance-Gremium darüber, wer Steward wird~\cite{torongo2023} -, weshalb ein Threshold Key als \texttt{submitKey} sie am genauesten abbildet: Er überträgt das kollektive Entscheiden mehrerer Steward-Instanzen unmittelbar auf den Signaturmechanismus und wird netzwerkseitig durchgesetzt, ohne dass dafür eine zusätzliche Komponente nötig wäre. Die delegierte zweite Stufe lässt sich damit jedoch nicht ausdrücken, da der \texttt{submitKey} nur zwischen berechtigt und unberechtigt unterscheidet und keine Rollen kennt. Eine rein anwendungsseitige Prüfung wiederum verschiebt die Durchsetzung vollständig auf die auswertende Komponente und gibt die netzwerkseitige Verankerung der Wurzel auf, womit die von der Referenzarchitektur geforderte Vertrauensanker-Funktion~\cite{erler2023} allein von der korrekten Implementierung der Leseseite abhängen würde. Die Kombination beider Kontrollpunkte bildet das belegte zweistufige Muster am genauesten ab und erfüllt FA1 sowie NFA7 am vollständigsten: Beide Stufen liefern ein beobachtbares Ergebnis, da eine unberechtigte Nachricht entweder vom Netzwerk abgewiesen oder von der auswertenden Komponente verworfen wird.

Beide Kontrollpunkte sind mit dieser Zuordnung noch nicht vollständig bestimmt. Auf der Wurzelstufe ist jede Änderung des Steward-Kreises zugleich eine Änderung des \texttt{submitKey} und damit des Topics selbst, die nur mit dem \texttt{adminKey} möglich ist (Abschnitt~\ref{subsec:grundlagen-hcs-topics}): Bleibt dieser ungesetzt, ist der Kreis dauerhaft eingefroren; wird er als einzelner Schlüssel gesetzt, kann dessen Inhaber den \texttt{submitKey} ersetzen und die Schwellenregel aushebeln. Auf der zweiten Stufe sind die Topics der öffentlichen DIDs und der AnonCreds-Objekte offen beschreibbar, sodass die auswertende Komponente zwei Angaben benötigt, die aus den Kontrollpunkten selbst nicht folgen: ein Merkmal, an dem sie eine dortige Nachricht ihrem Schreiber zuordnet, und eine zeitliche Bezugsgröße, gegen die sie den Delegationszustand prüft. Ohne beides ist die anwendungsseitige Prüfstufe nicht definiert und das Szenario S5 hätte für die Objekt-Topics kein Prüfkriterium; beide Angaben werden deshalb in der Entscheidung festgelegt.
...
\paragraph{Entscheidung.}
Die Vertrauenshierarchie wird auf einem einzelnen Topic geführt, dessen \texttt{submitKey} als Threshold Key von den Steward-Instanzen gemeinsam kontrolliert wird und die Registrierung neuer Akteure netzwerkseitig absichert; der \texttt{adminKey} wird mit derselben Schwellenregel belegt, damit der Steward-Kreis änderbar bleibt, ohne dass eine einzelne Instanz den \texttt{submitKey} ersetzen und die Wurzel übernehmen kann. Ob die Schwellenregel im Versuchsaufbau mit mehreren Schlüsseln oder mit einem einzelnen besetzt wird, ist für die gemessenen Eigenschaften ohne Belang und wird im Kapitel~\ref{cha:implementierung} festgelegt. Endorser-Rechte werden über Delegationsnachrichten auf demselben Topic vergeben und entzogen; da der \texttt{submitKey} den Schreibzugriff auf den Steward-Kreis beschränkt, ist ihre Herkunft bereits durch die Annahme der Nachricht durch das Netzwerk belegt, und die Hierarchie umfasst genau die von FA1 geforderten zwei Stufen, weil Endorser weder sich selbst entziehen noch weiter delegieren können. Die Gültigkeit prüft der Registry-Adapter als auswertende Komponente des in E5 festgelegten Lesewegs, indem er die Historie des Vertrauens-Topics bei jeder Prüfung erneut ausliest; geprüft wird gegen den Delegationszustand zum Konsens-Zeitstempel der jeweils bewerteten Objektnachricht, sodass ein späterer Entzug bereits geschriebene Objekte nicht rückwirkend entwertet und das Verhalten dem von Indy entspricht~\cite{torongo2023}. Die Zuordnung einer Nachricht auf den offen beschreibbaren Topics zu ihrem Schreiber muss ohne Eingriff in die durch E3 und E4 vorgegebenen Nachrichtenformate möglich sein; welches der dort verfügbaren Merkmale dafür herangezogen wird, ist eine Eigenschaft der gewählten Methoden und wird im Kapitel~\ref{cha:implementierung} festgestellt. Ist der Delegationszustand nicht lesbar, wird die Nachricht als ungültig behandelt. Die in FA5 geforderte topicübergreifende Historie wird aus den Konsens-Zeitstempeln der beteiligten Topics rekonstruiert; der dafür nötige Aufwand geht in die Messung nach NFA2 und NFA3 ein.
...
\footnotesize
  ...
  \begin{tabular}{p{0.19\textwidth} p{0.44\textwidth} p{0.24\textwidth}}
    ...
    \midrule
    E1 - Netzwerkbetriebsmodell und Vertrauensgrenze & Selbst betriebenes, mehrknotiges Hashgraph-Netzwerk mit eigener Mirror Node unter Kontrolle der beteiligten Institutionen & NFA6, NFA8; Vergleichbarkeit der Messung (NFA1, NFA2, NFA4) \\
    E2 - Speicherform der Identitätsobjekte & Vollständige Kodierung der Identitätsobjekte auf dem Ledger & FA3, FA4, FA5; NFA5, NFA8 \\
...
Für das Zusammenspiel dieser Komponenten ist eine Eigenschaft festzuhalten, die die HCS-basierte Variante von der Referenzvariante unterscheidet: Auf dem Schreibweg ist keine zustandsführende Komponente beteiligt. Das Netzwerk ordnet die Nachricht, versieht sie mit einem Konsens-Zeitstempel und stellt sie über die Mirror Node bereit. Eine Berechtigungsprüfung findet netzwerkseitig ausschließlich auf dem Vertrauens-Topic statt, dessen Schreibberechtigung nach E6 gesetzt ist; die Topics der öffentlichen DIDs und der AnonCreds-Objekte sind dagegen offen beschreibbar. Ob ein schreibender Akteur zum Zeitpunkt des Schreibens delegiert war, wird deshalb erst beim Lesen ausgewertet (E5). Der Aufwand des Lesewegs wächst dadurch mit der Länge der Topic-Historie - genau der Zusammenhang, den die Evaluation nach NFA2 und NFA5 quantifiziert; zugleich entsteht die Asymmetrie gegenüber Indy, das den unberechtigten Schreibvorgang bereits abweist, die das Szenario S5 untersucht.
...
\footnotesize
  ...
  \begin{tabular}{p{0.14\textwidth} p{0.33\textwidth} p{0.19\textwidth} p{0.16\textwidth}}
    ...
    NFA7 - Zugriffskontrolle & Ergebnis nicht autorisierter Schreibversuche, getrennt nach netzwerkseitiger und anwendungsseitiger Prüfung; für den Widerruf zusätzlich Verifikationsergebnis vor und nach dem Widerruf sowie Zeit bis zur Wirksamkeit, sofern die Symmetriebedingung nach Abschnitt~\ref{subsec:eval-validitaet} erfüllt ist & VDR-Schnittstelle & \cite{mamoru2026, torongo2023, tawfik2025} \\
...
\footnotesize
  ...
  \begin{tabular}{p{0.20\textwidth} p{0.52\textwidth} p{0.16\textwidth}}
    ...
    \midrule
    S1 - Basismessung & Sequenzielle Ausführung je einer Operation pro Ledger-Funktion (DID-Registrierung, Schema, Credential Definition, Leseoperationen) unter definierter Grundlast & NFA1, NFA2 \\
...
Die Sicherheitsdimension wird zweigeteilt bewertet. Experimentell prüfen die Szenarien S4 und S5 die Wirksamkeit der Konsens-Fehlertoleranz und der Schreibautorisierung. Analytisch wird die HCS-basierte Ledger-Schicht mit derselben STRIDE-Methodik untersucht, mit der Erler et al. die Referenzarchitektur entworfen haben~\cite{erler2023}; bewertet wird dabei nicht das Gesamtsystem, sondern ausschließlich die Differenz: welche Bedrohungen durch den Austausch der Ledger-Schicht entfallen, welche unverändert bestehen und welche neu hinzukommen. Die dafür nötigen Grundlagen werden nicht neu erhoben: Das Datenflussdiagramm, die Schutzgüter, die Vertrauensstufen und die Vertrauensgrenzen der Referenzarchitektur werden unverändert von Erler et al. übernommen~\cite{erler2023}; ausgetauscht wird ausschließlich das Element der Ledger-Schicht sowie diejenigen Datenflüsse, die die in Abschnitt~\ref{subsec:eval-messgroessen} festgelegte Messgrenze überschreiten. Ein eigenständiges Bedrohungsmodell des Gesamtsystems entsteht damit nicht; das bestehende Modell wird an genau der Stelle fortgeschrieben, an der der Entwurf von der Referenzarchitektur abweicht. Das Ergebnis wird je STRIDE-Kategorie in der Form entfallen, unverändert oder neu berichtet und, soweit vorhanden, durch den experimentellen Befund aus S4 und S5 belegt. Ein besonderes Augenmerk liegt auf den durch die vollständige Kodierung der Identitätsobjekte (E2) über die Mirror Node öffentlich sichtbaren Metadaten~\cite{satybaldy2026}. Die zweistufige Berechtigungsprüfung nach E6 wird dabei gesondert betrachtet, da die Wurzelstufe netzwerkseitig über den \texttt{submitKey} durchgesetzt wird, die Delegationsstufe dagegen anwendungsseitig im Registry-Adapter und damit außerhalb der Garantien des Konsensnetzwerks liegt.
...
\begin{itemize}
  ...
  \item Der Speicherbedarf je Konsensknoten nach NFA5 wird in beiden Varianten erhoben, ist jedoch keine symmetrische Größe. Die Konsensknoten von HCS verwerfen den Nachrichteninhalt nach der Ordnung (E5), sodass dort kein bestandsabhängiges Wachstum zu erwarten ist, während die Validatorknoten von Indy den abgeleiteten Zustand replizieren. Der Unterschied ist ein Ergebnis des Entwurfs und kein Messartefakt und wird entsprechend berichtet.
  \item Die gemessenen Lesewerte gelten für den in E5 gewählten Leseweg. Ein lokal fortgeschriebener Zustand nach dem Appnet-Muster würde die Abhängigkeit von der Historienlänge beseitigen; das damit nicht ausgeschöpfte Optimierungspotenzial wird im Kapitel~\ref{sec:discussion} anhand der gemessenen Werte beziffert.
\end{itemize}
