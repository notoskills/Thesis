...
\footnotesize
  ...
  \begin{tabular}{p{0.34\textwidth} p{0.56\textwidth}}
    ...
    Der Containerlaufzeit zugewiesen (WSL2) & 14 logische Prozessoren (Threads), 21,5\,GiB Arbeitsspeicher \\
...
\footnotesize
  \centering
  \caption{Startparameter des Agenten je Variante. Der Startwert der Trustee-DID -- der DID mit der höchsten Schreibrolle des Indy-Netzwerks -- ist gesetzt, wird hier jedoch nicht abgedruckt}
...
\footnotesize
  ...
  \begin{tabular}{p{0.08\textwidth} p{0.80\textwidth}}
    ...
    S4 & Eigene Last mit 5 parallelen Nutzern und Anfrageintervall 5,0\,s (nicht die Grundlast). Ausfall nach 120\,s Vorlauf, Ausfallfenster 180\,s, Beobachtung der Erholung über 300\,s bzw. 600\,s. Zielknoten: Knoten~2 auf der Indy-Variante, Knoten~1 auf der HCS-Variante, der zugleich der einzige ist, den die Client-Bibliothek anspricht (Abschnitt~\ref{sec:impl-befunde}, Befund B3); ein Ausfall eines anderen HCS-Knotens bliebe wirkungslos. Fünf Wiederholungen je Variante \\
...
\footnotesize
  ...
  \begin{tabular}{p{0.14\textwidth} p{0.33\textwidth} p{0.19\textwidth} p{0.16\textwidth}}
    ...
    NFA2 -- Latenz & Dauer von der Einreichung bis zur Finalität (Schreiben) bzw. von der Anfrage bis zur Antwort (Lesen), berichtet als Median und robuste Streuung & VDR-Schnittstelle & \cite{manoj2022, torongo2023, mamoru2026, satybaldy2026} \\
    ...
    NFA5 -- Datenwachstum & Lesedauer eines festen Zielobjekts und einer Kontrolllesung in Abhängigkeit von der Anzahl verwalteter Identitätsobjekte; Speicherbedarf je Konsensknoten und je Mirror Node vorgesehen, jedoch nicht erhoben & Konsensknoten, Mirror Node, VDR-Schnittstelle & \cite{mazlan2020, giao2026, mamoru2026, farin2025} \\
    NFA6 -- Konsens-Fehlertoleranz & Verfügbarkeit und Latenz bei Ausfall von $f$ Knoten, wobei $f$ die Zahl der tolerierbaren fehlerhaften Knoten bezeichnet (bei vier Knoten $f=1$), sowie Dauer bis zur Wiederaufnahme des Normalbetriebs & Konsensnetzwerk & \cite{hashim2021, verma2024, farin2025} \\
    NFA7 -- Zugriffskontrolle & Ergebnis nicht autorisierter Schreibversuche -- auf der HCS-Variante gegen den objektgebundenen Schlüsselschutz, auf der Indy-Variante gegen das Rollenmodell (die Vertrauenshierarchie nach E6 ist nicht umgesetzt, siehe Abschnitt~\ref{subsec:e6-vertrauenshierarchie}); für den Widerruf zusätzlich die Schreib- und Lesedauer des Widerrufspfads & VDR-Schnittstelle & \cite{mamoru2026, torongo2023, tawfik2025} \\
    \bottomrule
  \end{tabular}
\end{table}